...
If you have checked the official website of gui, you will find that gui's components do not dynamically insert `<style />` to control styles, but use CSS files to control styles:
...
CSS-in-JS has been criticized for its runtime performance loss. In gui, if your application uses SSR, you can skip the stage of generating styles at runtime on the client side to improve performance. Of course, we will continue to follow up the development of CSS-in-JS to bring you a better experience.
